...
- 時刻は **JST**。東証の大引けは **15:00**、シグナル監視は 9:00–15:30 平日（🔧 祝日も除外、`jpholiday` による休場日判定 `_is_trading_day_jst` を `run_picks_task`/`run_portfolio_monitor_task` に適用済み。手動トリガー `POST /api/picks/run` は意図的にガード対象外）。
...
- **外部公開物（note 記事・図表・スクリーンショット）では 3 値を検証用語で書く**（2026-09-25 金商法対応、ユーザー指示）: 推奨買値→シミュレーション起点価格、推奨損切値→シナリオ無効化水準、推奨売値→想定レンジ上限、推奨銘柄／ピック銘柄→アルゴリズム抽出銘柄／検証対象銘柄、強気／弱気（買い／売り）→正／負のトレンド相関（検証用）。置換表と記事末尾の定型免責ブロックの単一情報源は `backend/services/notes/compliance_terms.py`（自動下書き `note_generator.py`・図表 `scripts/generate_note_charts.py`・スクショのラベル差し替え `scripts/relabel_screenshot.py` が共有）。アプリ本体（model・API・UI・通知）の 3 値表記は上記のとおり変えない。記事の構成・画面キャプチャの扱いは `NOTE.md` に従う。
...
- AIピック判定のプロンプトも champion / challenger で扱う（`PICK_PROMPT_CHALLENGER`、`plans/05` §2h）。挑戦者は同じモデル・別プロンプトで並走し `prediction_ledger` に `is_shadow=1` で記録する。**台帳を読むコードは既定で `is_shadow=0` に絞る**（一覧・通知・note・確度較正・E3 ゲート・評価画面。挑戦者を含めて読むのは昇格評価だけ）。昇格が提案されてもレジストリの apply では実際のプロンプトは変わらない（コードの既定の版を人が変更する）。
...
- トレンド分析（`services/data/trend/analyzer.py`、feature `trend_analyzer`）も外部由来の生ニュース見出し（yfinance、未検証）を読むため同じ懸念が生じる。`analyzer.py` 自体は見出しを読んで `theme_name`/`summary`/`correlation_rationale` 等の自由記述込みで `Trend` を構造化するが、`services/data/trend/context.py`（`render_trends`）が `propose_stock_pick` 本体のプロンプトへ転送する前に **数値・enum・検証済み銘柄コードのみ**（`sentiment_score`/`momentum_score`/`lifecycle_stage`/`impact_horizon`/`related_tickers.ticker`）に絞り、自由記述は一切通さない（2026-09-18 に発見・修正。修正前は `summary` 等がそのまま転送されており `llm_news_sentiment_service` と同水準の防御が欠けていた。`plans/05_決定ログと未決事項.md` 変更履歴参照）。`theme_name`/`summary` は `/api/trend`（UI 表示専用）にはそのまま残る。
...
- 主要画面: ダッシュボード / 銘柄詳細（AI 思考トレース）/ チャート（🆕、AI ピック対象外も含め任意銘柄の株価チャートを素早く閲覧）/ ポートフォリオ / モデルラボ / 通知センター。

## 自律運用

- celery-beat がピック生成（JST 07:00）/ note下書き自動生成・Vaultアーカイブレポート生成（08:05/08:15）/ 保有監視（場中 5 分周期）/ 決着解決・評価・軽量再学習・PSI（夜間）/ フル再学習 + 昇格ゲート（週次）/ EOD レビュー（16:31）を自動発火。→ **手動で `.delay()` タスク投入しない**。
...
- **Claude から起動するプロセスは必ず Claude のプロセスツリーから切り離す**（2026-09-26 ユーザー指示）: Bash/PowerShell の `run_in_background` や `Start-Process` で起動すると Claude の子プロセスになり、**セッション終了時に巻き添えで停止する**（2026-09-25 17:44 のセッション終了で celery worker/beat が停止し、翌朝まで定期タスクが止まった）。WMI 経由で起動すれば親が `WmiPrvSE.exe` になり切り離される。ログは `start.ps1` と同じ `logs/` へ出す:
  ```powershell
  Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = 'cmd.exe /c ".venv\Scripts\celery.exe -A backend.celery_app worker --pool=solo --loglevel=info >> logs\celery.out.log 2>> logs\celery.err.log"'; CurrentDirectory = (Get-Location).Path }
  ```
  （beat は `worker --pool=solo` → `beat`、ログ名 `celery-beat.*.log` に置き換える。frontend/backend を Claude から起動する場合も同様。）
- **celery worker**: `.venv/Scripts/celery.exe -A backend.celery_app worker --pool=solo --loglevel=info` を上記の切り離し方式で起動（Windows では `--pool=solo` 必須）。ピック生成・保有監視等のタスクを裏で実行するため常時稼働させる。
