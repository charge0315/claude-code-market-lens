...
│                  inference / portfolio / notify / stock
├── services/
│   ├── data/      jquants / yfinance / trend
│   ├── vault/     brand_notes / news_digest / daily_note
│   ├── scoring/   technical / fundamental / sentiment / recommender
│   ├── picks/     pipeline / bracket / *_prompt
│   ├── ledger/    prediction_ledger / outcome_resolver / eval_service / weekly_learning_service
│   ├── learning/  feature_engineering / pool_model / pool_training_service
│   ├── registry/  promotion / drift / calibration / factor_weight_service
...
├── components/    ui / layout / pipeline / dashboard / portfolio / notify / model-lab / stock-detail
└── lib/           api/（REST クライアント）/ pipeline/ / realtime/（SSE・WS）/ push/
```
