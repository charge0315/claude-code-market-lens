...
1. **「システム起動して」プロトコル**:
   - 起動確認順序: Redis (Docker) → kb_creator (8077) → Backend (8002) → Frontend (3001) → Celery Worker (`--pool=solo`) → Celery Beat。
   - 既に動いているプロセスは二重起動せず、落ちているもののみを選択して起動する。
2. **Celery Beat の自動起動**（🔧2026-09-22、旧「絶対禁止」方針から変更）:
   - `celery-beat` も通常の「システム起動」対象に**含める**（確認不要で起動してよい）。
   - **経緯**: 2026-09-14 に「初回起動時、実行履歴欠落により過去の全定期タスクが一斉に『未実行』とみなされて同時発火し、ピックの二重生成やリソース枯渇を引き起こす」インシデントが発生し一時的に自動起動禁止としていたが、根本原因（`run_picks_task` に休場日判定・当日重複防止ガードが無かったこと）は `run_picks_task` への `_is_trading_day_jst`（`jpholiday`込み）＋当日分既発行チェックの追加で解消済み（2026-09-20）。加えてユーザー自身が `scripts/register-startup-task.ps1`（`AlphaForge-Autostart` タスク）でログオン時に celery beat を含む4プロセス一括自動起動する運用へ既に移行している。週次タスク（ドリフト検知・昇格評価）は同日重複防止ガードを持たないが、提案書き込みのみで実売買・自動昇格には影響しないため許容している。
