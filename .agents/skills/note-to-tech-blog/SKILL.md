...
## ✍️ note記事と共通の書き方ルール（2026-09-25 ユーザー指示）

note.com の記事と同じルールを Qiita/Zenn 版にも当てはめる（`NOTE.md` 参照）。

1. **「# はじめに」のような挨拶の見出しを付けない**。本文はエンジニアに刺さる一言から始める（例:「締め切りが『相場の開始時刻』で決まっているバッチを、書いたことはありますか。」）。
2. **要約は番号付きリスト**にする（`:::message`／`:::note info` の中で `1.` `2.` `3.`）。手順や時系列（①〜⑥、日付順の経緯など）も番号付きリストにする。
3. **画像には必ずキャプションを付ける**。画像の直後の行に `*キャプション*` を書く（Zenn はキャプションとして表示される）。Zenn の画像は `docs/zenn/images/<記事スラッグ>/` に置き、`/images/<記事スラッグ>/ファイル名` で参照する。Qiita はエディタで画像をアップロードして URL を差し替える前提で、ファイル名だけを書いておく。
4. **定型免責ブロックを末尾に入れ、本文は3項目の箇条書きにする**（`compliance_terms.COMPLIANCE_DISCLAIMER` と同じ文面）。技術記事用の短い免責事項の後ろに、`:::message`／`:::note info` として置く。
5. 検証用語（シミュレーション起点価格など）と方向ラベル（正／負のトレンド相関（検証用））を使い、「強気／弱気」「推奨銘柄」などのNG表現を使わない。
